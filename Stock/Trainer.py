...
class Trainer(object):
    ...
    def train_epoch(self, epoch):
        self.model.train()
        total_loss = 0
        for batch_idx, (ori_data, MPG, target, label, use_data) in enumerate(self.train_loader):
    ...
    def test(model, args, data_loader, scaler, logger, path=None):
        ...
        y_true = []
        with torch.no_grad():
            for batch_idx, (ori_data, MPG, target, label, use_data, use_t, use_i) in enumerate(data_loader):
