...
class PSCN():
    def __init__(self,w,s=1,k=10
        ...
        labeling_procedure_name : the labeling procedure for ranking the nodes between them. ATS and betweeness centrality is implemented.
        ...
        step_max : the max step of ATS algorithm
        """
        self.w=w
        self.s=s
        self.k=k
    ...
    def create_model(self):
       model=Sequential()
       model.add(Conv1D(filters=1,kernel_size=self.k,strides=self.k,input_shape=(self.w*self.k,self.attr_dim)))    
       model.add(Flatten())
       model.add(Dense(32,activation="relu",name='embedding_layer'))
    ...
    def process_data(self,X,y=None,ATS_list = None): # X is a list of Graph objects
        start=time.time()
        n=len(X)
        train=[]
        ...
        for i in range(n):
            if(self.labeling_procedure_name != 'ATS' or ATS_list is None):
                ...
                rfMaker=ReceptiveFieldMaker(X[i].nx_graph,w=self.w,ATS = ATS_list[i],k=self.k,s=self.s
                                            ,labeling_procedure_name=self.labeling_procedure_name
                                            ,use_node_deg=self.use_node_deg,one_hot=self.one_hot,dummy_value=self.dummy_value,step_max = self.step_max)
    ...
    def fit(self,X,y,ATS_list):
        
        if not self.use_preprocess_data:
            X_preprocessed,y_preprocessed=self.process_data(X,y,ATS_list)
        else:
            X_preprocessed,y_preprocessed=self.process_data(X,y)
        train = {'x': X_preprocessed, 'y': y_preprocessed}
        with open(os.path.join(up_file, 'usedataset', 'close_train_yb1.pickle'), 'wb') as fp:
    ...
    def predict(self,X,ATS_list):
        
        if not self.use_preprocess_data:
            X_preprocessed=self.process_data(X,ATS_list = ATS_list)
        else:
            X_preprocessed=self.process_data(X)
        
        return self.model.predict(X_preprocessed).ravel(),X_preprocessed
    
    def return_embedding(self,X,ATS_list = None):
        X_preprocessed = self.process_data(X,ATS_list = ATS_list)
...
class ReceptiveFieldMaker():
    def __init__(self,nx_graph,w,ATS=None,s=1,k=10,labeling_procedure_name='betweeness',use_node_deg=False,one_hot=False,dummy_value=-1,step_max = 20,time_hapram = 10):
        ...
        self.s = s
        self.k = k
        self.time_hapram  = w#time_hapram 
        self.dummy_value = dummy_value
        ...
        if self.labeling_procedure_name=='approx_betweeness':
            ...
            self.all_times['first_labeling_procedure'].append(end-st)
        elif self.labeling_procedure_name=='ATS':
            st=time.time()
            self.dict_first_labeling=self.get_ATS_label(self.ATS,self.nx_graph) 
            end=time.time()
            self.all_times['first_labeling_procedure'].append(end-st)
        else :
            st=time.time()
    ...
    def make_(self):
        ...
        forcnn=[]
        self.all_subgraph=[] 
        f,_f=self.select_node_sequence()
        self.all_ori_sub_graph = _f
        for graph in f :
    ...
    def labeling_procedure(self,graph,ATS = None):   
        st=time.time()
        if (self.labeling_procedure_name=='betweeness' or (ATS is None)):
            a=self.betweenness_centrality_labeling(graph)
        elif(self.labeling_procedure_name=='ATS'):
            a=self.get_ATS_label(ATS,graph)
        end=time.time()
...
#####这里是和点的标签无关的########
    def get_ATS_label(self,x,graph):
        result={}
        labeled_graph=nx.Graph(graph)
        last_b = np.arange(len(x))
        label_list = np.zeros([len(x)])
        for _step in range(self.step_max):
            try:
                a = ATS(x,step=_step)
                b = [int(x[0]) for x in np.asarray(a[:,0]).tolist()]
            except Exception as e:
                #print(_step,e)
                b = copy.copy(last_b)
            for _b in last_b:
                if(_b not in b):
                    label_list[_b] = _step/self.step_max
            last_b = copy.copy(b)
        label_list[-self.time_hapram:] = 1.01
        label_tuple = []
        for label,node_i in zip(label_list[::-1],list(labeled_graph.nodes())[::-1]):
            label_tuple.append((node_i,label))

        sorted_centrality=sorted(label_tuple,key=lambda n:n[1],reverse=True)
    ...
    def select_node_sequence(self):
        ...
        while j<=self.w :
            if i<len(Vsort):
                ng,sg = self.receptiveField(Vsort[i])
                f.append(ng)
                _f.append(sg)
            else:
    ...
    def receptiveField(self,vertex):
        ...
        #print('norm',normalized_subgraph)
        ed2=time.time()
        self.all_times['normalized_subgraph'].append(ed2-ed)


        return normalized_subgraph,subgraph
